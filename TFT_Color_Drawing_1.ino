...
void loop() {
  ...
  if (p.z > MINPRESSURE && p.z < MAXPRESSURE) {
    ...
    if (thickness > 0) { //Wait for user to select a thickness setting
...
void checkButtons() {
  ...
  if (y >= 0 && y < 40) { //Brown
    ...
    drawMenu();
    resetButton();
    tft.fillCircle(25, 20, 5, BLACK);
  } else if (y >= 40 && y < 80) { //Red
    color = RED;
    drawMenu();
    resetButton();
    tft.fillCircle(25, 60, 5, BLACK);
  } else if (y >= 80 && y < 120) { //Green
    color = GREEN;
    drawMenu();
    resetButton();
    tft.fillCircle(25, 100, 5, BLACK);
  } else if (y >= 120 && y < 160) { //Blue
    color = BLUE;
    drawMenu();
    resetButton();
    tft.fillCircle(25, 140, 5, BLACK);
  } else if (y >= 160 && y < 200) { //Magenta
    color = MAGENTA;
    drawMenu();
    resetButton();
    tft.fillCircle(25, 180, 5, BLACK);
  } else if (y >= 200 && y < 240) { //Yellow
    color = YELLOW;
    drawMenu();
    resetButton();
    tft.fillCircle(25, 220, 5, BLACK);
  } else if (y >= 240 && y < 280) { //Cyan
    color = CYAN;
    drawMenu();
    resetButton();
    tft.fillCircle(25, 260, 5, BLACK);
  } else if (y >= 280 && y < 320) { //White
    color = WHITE;
    drawMenu();
    resetButton();
    tft.fillCircle(25, 300, 5, BLACK);
  }
  }
